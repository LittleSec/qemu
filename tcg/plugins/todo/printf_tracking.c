...
int check_format_validity(target_ulong pc, uint32_t format_addr)
{
    memory_range *m = mappings;
    for (; m != NULL; m = m->next)
    {
        if (m->address > format_addr)
            return 0;
        if ((m->address < format_addr) && (format_addr < m->address + m->size))
            return (m->perms & 2) ? 0 : 1;
    }
    return 0;
}

// void on_call_printf_cb(CPUState *cpu, target_ulong pc)
void on_call_printf_cb(CPUArchState *env, target_ulong pc)
{
